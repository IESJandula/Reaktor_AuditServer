...
public interface IAuditoriaRepository extends JpaRepository<Auditoria, Long>
{
	 ...
	 */
	@Query(value = "SELECT DAYOFWEEK(timestamp), COUNT(*) FROM auditoria WHERE curso_academico = :cursoAcademico GROUP BY DAYOFWEEK(timestamp)",
		   nativeQuery = true)
	 ...
	 */
	@Query(value = "SELECT HOUR(timestamp), COUNT(*) FROM auditoria WHERE curso_academico = :cursoAcademico GROUP BY HOUR(timestamp)",
		   nativeQuery = true)
